...
export const getMessageSelectKey = (msg?: Pick<V2NIMMessageForUI, 'messageClientId' | 'messageServerId' | 'createTime' | 'senderId' | 'conversationId'>) => {
  if (!msg) return ''
  if (msg.messageServerId && msg.messageServerId !== '0') return `server-${msg.messageServerId}`
  if (msg.messageClientId) return `client-${msg.messageClientId}`
  return `local-${msg.conversationId}-${msg.senderId}-${msg.createTime}`
}

export const isSelectableMessage = (msg?: V2NIMMessageForUI & { timeValue?: number }) => {
  return !!(
    msg &&
    msg.messageClientId &&
    !msg.timeValue &&
...
export const isMessageStatusFailed = (msg?: V2NIMMessageForUI) => {
  const errorCode = msg?.messageStatus?.errorCode
  return errorCode !== undefined && errorCode !== 200
}

export const isForwardableMessage = (store: RootStore, msg: V2NIMMessageForUI) => {
  if (
    !isSelectableMessage(msg) ||
  ...
  if (msg.messageType === V2NIMConst.V2NIMMessageType.V2NIM_MESSAGE_TYPE_CUSTOM) {
    return isMergedForwardMsg(store, msg)
  }
...
export const syncConversationLastMessageAfterDelete = (store: any, conversationId: string) => {

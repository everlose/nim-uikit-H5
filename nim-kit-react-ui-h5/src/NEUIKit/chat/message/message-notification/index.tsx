 ...
 * 通知消息组件
 *
 * 通知内容在 render 阶段直接计算（而非 useEffect），这样 observer() 可以自动追踪
 * getAppellation 中访问的 MobX observable 依赖（群昵称/好友别名/用户名等），
 * 当用户修改群昵称后通知消息中的昵称能及时更新。
 */
const MessageNotification: React.FC<MessageNotificationProps> = observer(({ msg }) => {
  ...
  // 预拉取通知中涉及的用户信息（仅 targetIds 场景需要，确保 getAppellation 有数据可用）
  useEffect(() => {
    const attachment = msg.attachment as V2NIMMessageNotificationAttachment
    const prefetchTypes = [
      V2NIMConst.V2NIMMessageNotificationType.V2NIM_MESSAGE_NOTIFICATION_TYPE_TEAM_INVITE,
      V2NIMConst.V2NIMMessageNotificationType.V2NIM_MESSAGE_NOTIFICATION_TYPE_TEAM_KICK,
      V2NIMConst.V2NIMMessageNotificationType.V2NIM_MESSAGE_NOTIFICATION_TYPE_TEAM_ADD_MANAGER,
      V2NIMConst.V2NIMMessageNotificationType.V2NIM_MESSAGE_NOTIFICATION_TYPE_TEAM_REMOVE_MANAGER,
    ]
    if (prefetchTypes.includes(attachment?.type)) {
  ...
  // 直接计算通知内容（observer 自动追踪 MobX 依赖，昵称变更会触发重渲染）
    ...
    switch (attachment?.type) {
      ...
      case V2NIMConst.V2NIMMessageNotificationType.V2NIM_MESSAGE_NOTIFICATION_TYPE_TEAM_INVITE: {
        ...
        const nicks = accounts
          .map((item) => store.uiStore.getAppellation({ account: item, teamId }))
          .filter((item) => !!item)
          .join('、')

...
export const isCollectionLimitError = (error: unknown) => {
  ...
  return (
    err?.code === COLLECTION_LIMIT_ERROR_CODE ||
    err?.errorCode === COLLECTION_LIMIT_ERROR_CODE ||
    err?.messageStatus?.errorCode === COLLECTION_LIMIT_ERROR_CODE
  );
};

export const canCollectMessage = (msg: V2NIMMessageForUI) => {
  if (!msg || msg.recallType || msg.isDelete) {
    return false;
  }
  if (!SUCCESS_SENDING_STATES.has(msg.sendingState)) {
    return false;
  }
...
export const getMessageCollectionConverter = (
  nim: unknown
): MessageCollectionConverter | null => {
  return (
    (nim as { V2NIMMessageConverter?: MessageCollectionConverter })
      ?.V2NIMMessageConverter || null
  );
};
...
export const parseMessageCollection = (
  collection: V2NIMCollection,
  converter?: MessageCollectionConverter | null
): MessageCollectionItem | null => {
  if (collection.collectionType !== MESSAGE_COLLECTION_TYPE) {
    return null;
  }

  const serializedMsg = converter?.messageDeserialization(
    collection.collectionData || ""
  );
  if (
    serializedMsg?.messageClientId &&
    serializedMsg.conversationId &&
    typeof serializedMsg.messageType === "number"
  ) {
    return {
      collection,
      msg: serializedMsg as V2NIMMessageForUI,
      collectionTime:
        collection.createTime || collection.updateTime || serializedMsg.createTime || 0,
    };
  }
  ...
  try {
    const payload = JSON.parse(
      collection.collectionData || "{}"
    ) as Partial<MessageCollectionPayload>;
...
export const normalizeMessageCollections = (
